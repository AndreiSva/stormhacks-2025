    ...
    for (let j = 0; j < height; j++) {
      ...
      if (Math.abs(tileX) + Math.abs(tileY) <= genDistance) {
        ...
        if (camera) {
          ...
          const tileDistance = camera.position.distanceTo(
            new THREE.Vector3(tileCenterX, tileCenterY, 0)
          );
...
export function enableFog( scene: THREE.Scene, renderDistance: number, fogColor: number = 0x87CEEB) // Optional: set default fog color to sky blue )
{
